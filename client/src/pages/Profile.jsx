...
export default function Profile() {
  ...
  // decode token from response server to fetch the user id
  const { id } = jwt(state.user.token) 

  // fetch profile data from server using useQuery
  let { data: profile } = useQuery('profileCache', async () => {
    const response = await API.get(`/profile/${id}`);
    return response?.data?.data;
  });

  // fetch transaction data from server using useQuery
